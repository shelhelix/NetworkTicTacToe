...
public class ShapesConfigWindow : EditorWindow {
	...
	void OnGUI() {
		...
		using( var scroll = new GUILayout.ScrollViewScope( scrollPos, false, false ) ) {
			...
			using( ShapesUI.Group ) {
				...
				EditorGUILayout.PropertyField( useImmediateModeInstancing, new GUIContent( "Use GPU Instancing" ) );
			}

			using( ShapesUI.Group ) {
				GUILayout.Label( "Immediate-mode polylines & arcs", EditorStyles.boldLabel );
				EditorGUILayout.PropertyField( polylineDefaultPointsPerTurn, new GUIContent( "Default points per turn" ) );
				EditorGUILayout.PropertyField( polylineBezierAngularSumAccuracy, new GUIContent( "Angular sum accuracy" ) );
			}
	...
	void ApplyShaderSettings() {
		string path = ShapesIO.ShaderFolder + "/Shapes Config.cginc";
