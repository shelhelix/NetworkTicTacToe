...
namespace Shapes {
	...
	public static class CodegenDrawOverloads {
		...
		const string DOC_REGPOL = "Draws a regular polygon with the given number of sides";
		...
		const string DOC_DISC = "Draws a solid filled disc with a given radius";
		const string DOC_RING = "Draws a ring (circle) with a given radius and thickness";
		const string DOC_PIE = "Draws a solid filled pie shape, with a given radius and start/end angles";
		const string DOC_ARC = "Draws a circular arc, with a given radius, thickness, and start/end angles";
		...
		static List<string> GenerateAllOverloads() {
			...
			OrSelectorParams rotIdentityNormalQuat = new OrSelectorParams(
				null,
				new[] { new Param( $"Vector3 normal // {DOC_NORMAL}", "rot" ) { methodCallStr = "Quaternion.LookRotation( normal )" } },
				new[] { new Param( $"Quaternion rot // {DOC_ROT}" ) }
			);

			// Line
			foreach( ( bool dashed, string suffix ) in new[] { ( false, "" ), ( true, "Dashed" ) } ) {
				g = new OverloadGenerator( "Line" + suffix, callTargets["Line"], DOC_LINE ) { objectName = "line" };
				if( dashed == false )
			...
			// Polyline
			g = new OverloadGenerator( "Polyline", callTargets["Polyline"], DOC_POLYLINE );
			...
			foreach( ( int fillType, string fillSuffix ) in fillTypes ) {
				g = new OverloadGenerator( "Polygon" + fillSuffix, callTargets["Polygon"], DOC_POLYGON ) { objectName = "polygon" };
			...
			foreach( ( int fillType, string fillSuffix ) in fillTypes ) {
				foreach( ( bool hollow, string hollownessSuffix ) in new[] { ( false, "" ), ( true, "Hollow" ) } ) {
					g = new OverloadGenerator( "RegularPolygon" + hollownessSuffix + fillSuffix, callTargets["RegularPolygon"], DOC_REGPOL ) { objectName = "regular polygon" };
					g.constAssigns["hollow"] = hollow.ToString().ToLower();

					g += $"Vector3 pos // {DOC_POS}";
			...
			for( int idt = 0; idt < 4; idt++ ) {
				...
				for( int igr = 0; igr < 4; igr++ ) {
					foreach( ( bool dashed, string dashSuffix ) in new[] { ( false, "" ), ( true, "Dashed" ) } ) {
						...
						g = new OverloadGenerator( discType + gradientTypeSuffix[igr] + dashSuffix, callTargets[discType], shapeDoc[idt] ) { objectName = discType };
						if( dashed == false )
							g.constAssigns["dashStyle"] = "null"; // make sure it's always undashed
						g += $"Vector3 pos // {DOC_POS}";
						g += rotIdentityNormalQuat;
			...
			foreach( ( bool bordered, string suffixOvld ) in new[] { ( false, "" ), ( true, "Border" ) } ) {
				for( int mode = 0; mode < 4; mode++ ) {
					...
					g = new OverloadGenerator( "Rectangle" + suffixOvld, callTargets["Rectangle"], DOC_RECT ) { objectName = "rectangle" };
					if( bordered )
			...
			// Triangle
			g = new OverloadGenerator( "Triangle", callTargets["Triangle"], DOC_TRI );
			...
			// Quad
			g = new OverloadGenerator( "Quad", callTargets["Quad"], DOC_QUAD );
			g += $"Vector3 a // {DOC_A_POS}";
			g += $"Vector3 b // {DOC_B_POS}";
			g += $"Vector3 c // {DOC_C_POS}";
			...
			// Sphere
			g = new OverloadGenerator( "Sphere", callTargets["Sphere"], DOC_SPHERE );
			g += $"Vector3 pos // {DOC_POS}";
			...
			foreach( ( bool cube, string name ) in new[] { ( false, "Cuboid" ), ( true, "Cube" ) } ) {
				g = new OverloadGenerator( name, callTargets["Cuboid"], cube ? DOC_CUBE : DOC_CUBOID );
				g += $"Vector3 pos // {DOC_CUBOID_POS}";
				g += rotIdentityNormalQuat;
				if( cube )
			...
			// Cone
			g = new OverloadGenerator( "Cone", callTargets["Cone"], DOC_CONE );
			g += $"Vector3 pos // {DOC_CONE_POS}";
			g += rotIdentityNormalQuat;
			...
			// Torus
			g = new OverloadGenerator( "Torus", callTargets["Torus"], DOC_TORUS );
			g += $"Vector3 pos // {DOC_POS}";
			g += rotIdentityNormalQuat;
			...
			// Text
			g = new OverloadGenerator( "Text", callTargets["Text"], DOC_TEXT );
			g += $"Vector3 pos // {DOC_POS}";
			g += new OrSelectorParams(
				null,
				new[] { new Param( $"Vector3 normal // {DOC_NORMAL}", "rot" ) { methodCallStr = "Quaternion.LookRotation( normal )" } },
				new Param[] { $"Quaternion rot // {DOC_ROT}" },
				new[] { new Param( $"float angle // {DOC_TEXT_ANGLE}", "rot" ) { methodCallStr = "Quaternion.Euler( 0f, 0f, angle * Mathf.Rad2Deg )" } }
			);
